...
* The command key "I" increments the "counter" value, waits for the "reportRate" value in seconds and repeats this 10 times. The 'RepRate' column shows the "count down" from 10 to 0 in parentheses e.g. ```2s (10), 2s(9), 2s (8), etc```. When the "RepRate" counter reaches zero, the updates stop. 
  
  Each time the "I" command is issued, the "reportRate" will cycle through the series of values in seconds 0, 2, 5 & 10 then back to 0. When the "RepRate" is set or reaches zero, the "Count" is no longer being incremented.
  
   <img src="./assets/DemoI+.png" width="600"/>

* The command key "TAB", "CTRL+I" also works, replicates the "I" command key, but does not count down. Instead the "Count" value will increment by 1 **forever**. With this variation, the 'RepRate' column still shows the interval, 0s, 2s, 5s & 10s, but the "RepRate" count has been replaced by ```(oo)``` representing 'infinity', as an indicator of functionality difference.
  
   <img src="./assets/TabFunc.png" width="600"/><br>
  Because the counter increments _forever_, the MQTT connection may remain connected for a long period of time. Depending on the user's Azure account, this may incurring addition charges to the account holder.
